...
class Pose_record:
    ...
    def trajectory_service(self, key, *order):
        ...
        if not order:
            ...
            temp = order[0]
            twist.linear.x = temp.data[2]
            twist.linear.y = temp.data[3]
            twist.angular.z = temp.data[4]
            
            self.vel_srv_req.velocity.linear = twist.linear
            self.vel_srv_req.velocity.angular = twist.angular
            self.vel_srv_req.duration.data = temp.data[1]
...
class Human_avoidance:
    ...
    def callback(self, msg):
        ...
        if self.mydata.data[5] == 0:
            self.pose_srv.trajectory_service(55,self.mydata)
        else:
            self.pose_srv.trajectory_service(self.pose_srv.key_pressed)
            # print(self.pose_srv.key_pressed)
