...
def social_force(spot, ped):
    global spot_velo, s_force, follow_flag, people_nearby
    forces = 0
    force = 0
    lambda_ = 0.75
    x = 0
    ...
    for i in range(len(ped.objects)):
        distances[i,0] = ped.objects[i].label_id
        distances[i,1] = np.linalg.norm(np.array([[ped.objects[i].position[0]], 
    ...
    for i in range(len(ped.objects)):
        ...
        # If pedestrian is moving apply an elliptical force model
            ...
            ped_spot_ang = angle_between(unit_vector(ped_velo),unit_vector(spot_velo))

            x_ind,y_ind = np.unravel_index(np.argmin(distances, axis = None),distances.shape)

            if ped_spot_ang < 30 and distances[x_ind,0] == ped.objects[i].label_id and people_nearby == True
                ...
                # An approach here would be to just have the follow flag use the old follow method, 
                # but restrict the velocity to the velo of the person followed
