...
def social_force(spot, ped):
    global spot_velo, s_force, follow_flag, people_nearby, leader_pos, distances,
    global leader, leader_velo, leader_too_close, alone_count, delta_time
    forces = np.array([[0],[0]])
    force = np.array([[0],[0]])
    threshold = 0.1
    lambda_ = 0.75
    ...
    for i in range(len(ped.objects)):
        ...
        #create empty column to populate if leader is no longer to be followed
        past_leader.append(0)


        #x-coord for pedestrian[i]
        x_coord.append(ped.objects[i].position[0])
        #y-coord for pedestrian[i]
        y_coord.append(ped.objects[i].position[1])
        #x-velo for pedestrian[i]
        x_velo.append(ped.objects[i].velocity[0])
        #y-velo for pedestrian[i]
        y_velo.append(ped.objects[i].velocity[1])

    ####################################################################################
    # Creating matrix that remember past peoples positions (if they are coming closer) #
    ####################################################################################
    if np.sum(distances) != 0:
        old = np.array(np.isin(distances[:,0] == ids)).T
        np.append(distances, old)
        dist_old = np.delete(distances, np.where((distances[:,-1] == 1))[0], axis=0)
        dist_old = np.delete(dist_old, np.where((dist_old[:,1] > 3.0 ))[0], axis=0)
        dist_old = np.delete(dist_old, np.where((dist_old[:,2] == 0 ))[0], axis=0)
        dist_old = np.delete(dist_old, -1, axis=1)
        if np.sum(dist_old) != 0 :
            for n in range(len(dist_old[:,0])):
                dist_old[n,5] = dist_old[n,5] + dist_old[n,7] * delta_time  #new x-coord
                dist_old[n,6] = dist_old[n,6] + dist_old[n,8] * delta_time  #new y-coord
                dist_old[n,1] = (dist_old[n,5]**2 + dist_old[n,6]**2)**0.5  #new distance
    ...
    distances = np.array([ids, dists, to_or_away, less_than_4, past_leader, x_coord, y_coord, x_velo, y_velo]).T
    rospy.loginfo(distances)

    if old_flag == True:
        np.append(distances, dist_old, axis=0)
...
def callback(spot, obj_det):
    ...
    global follow_flag, leader_pos, rotation, leader, motion_count, delta_time
    allowed_velo = 1
    angle_to_goal = 0
    RelaxationTime = 0.5
    v_max = 1
