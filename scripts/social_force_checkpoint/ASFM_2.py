        ...
        # If pedestrian is moving above threshold apply an elliptical force model
            ...
            if ped_spot_ang*180/np.pi < 30 and distances[x_ind,0] == ped.objects[i].label_id and distances[x_ind,1] < 4.0 and leader == None and people_nearby == True and distances[x_ind,4] != 1:
                ...
                ####################
                    ...
                    ell_dis, x_ell, y_ell = ellipse_center_dist(ped_spot_ang, a, b)

                    repulsive_force = A * math.exp((distance-ell_dis)*B + C)

                    point_a = np.add(diff,unit_vector(ped_velo)*a)

                    new_source = np.add(diff,unit_vector(ped_velo)*x_ell)

                    force = np.array([[new_source[0,0]*repulsive_force],
                                      [new_source[1,0]*repulsive_force]])
...
def move(delta_time,socialforce, desiredforce, current_velo, allowed_v, follow_flag):
    global leader_velo, count, distances, a
    multiplier = 0
...
def callback(spot, obj_det):
    ...
    global follow_flag, leader_pos, rotation, leader, motion_count, a, avg_acc, avg_velo, exp_cnt, start_time, past_coord, dist_trav
    allowed_velo = 1
    angle_to_goal = 0
    RelaxationTime = 0.5
    v_max = 1
